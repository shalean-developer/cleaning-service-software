...
create table public.earning_lines (
  id uuid primary key default gen_random_uuid(),
  cleaner_id uuid not null references public.cleaners (id) on delete cascade,
...
create table public.booking_state_audit (
...
create index idx_bookings_status_scheduled_start
  on public.bookings (status, scheduled_start);

create index idx_bookings_customer_created_at
  on public.bookings (customer_id, created_at desc);

create index idx_bookings_cleaner_scheduled_start
...
create index idx_assignment_offers_booking_cleaner
  on public.assignment_offers (booking_id, cleaner_id);

create index idx_earning_lines_cleaner_created_at
