  ...
  return (
    ...
    >
      {CTA_TRUST_ITEMS.map((item) => (
        <li key={item} className="inline-flex items-center gap-1 text-[0.6875rem] text-slate-600">
  ...
  return (
    <div className="space-y-2.5">
      ...
      <header
        ...
        className="rounded-lg border border-slate-200/90 bg-shalean-soft-blue/50 px-3 py-2"
      >
        <p className="text-sm font-semibold text-shalean-navy">{serviceLabel}</p>
        <p className="mt-0.5 text-sm text-slate-700">{scheduleLabel}</p>
        {snapshotMeta.length > 0 ? (
          <p className="mt-0.5 text-xs leading-snug text-slate-600">{snapshotMeta.join(" · ")}</p>
        ) : null}
        {guestReadyNote ? (
      ...
      <section
        ...
        className="rounded-lg border border-slate-300/80 bg-white px-3 py-2.5 ring-1 ring-shalean-primary/5"
      >
        <p
        ...
        <p className="mt-0.5 text-3xl font-semibold tabular-nums tracking-tight text-shalean-navy">
          {formatZar(quote.totalCents)}
        </p>
        {amountHelper ? (
          <p className="mt-1 text-xs leading-snug text-slate-600">{amountHelper}</p>
        ) : null}
      </section>
