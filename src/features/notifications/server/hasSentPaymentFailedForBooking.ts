...
import { PAYMENT_FAILED_TEMPLATE } from "./config";
...
/**
 * True when another outbox row already delivered payment_failed for this booking.
 */
export async function hasSentPaymentFailedForBooking(
  ...
  for (const row of data ?? []) {
    ...
    if (readTemplate(row.payload) !== PAYMENT_FAILED_TEMPLATE) continue;
