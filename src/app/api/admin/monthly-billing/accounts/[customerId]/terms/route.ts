...
import { updateCustomerBillingTerms } from "@/features/monthly-billing/server/updateCustomerBillingTermsFacade";
import { parseUpdateMonthlyBillingTermsBody } from "@/features/monthly-billing/server/parseMonthlyBillingMutationBody";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const parsed = parseUpdateMonthlyBillingTermsBody(body);
  if (!parsed.ok) {
    return NextResponse.json(
  ...
  const result = await updateCustomerBillingTerms({
    admin: user,
    customerId,
