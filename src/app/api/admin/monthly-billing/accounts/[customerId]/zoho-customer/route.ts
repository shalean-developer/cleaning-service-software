...
import { linkCustomerZohoCustomer } from "@/features/monthly-billing/server/linkCustomerZohoCustomerFacade";
import { parseLinkZohoCustomerBody } from "@/features/monthly-billing/server/parseMonthlyBillingMutationBody";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const parsed = parseLinkZohoCustomerBody(body);
  if (!parsed.ok) {
    return NextResponse.json(
  ...
  const result = await linkCustomerZohoCustomer({
    admin: user,
    customerId,
