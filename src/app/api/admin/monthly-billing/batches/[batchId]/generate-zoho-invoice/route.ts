...
import { generateZohoMonthlyInvoice } from "@/features/monthly-billing/server/generateZohoMonthlyInvoiceFacade";
import { parseGenerateZohoMonthlyInvoiceBody } from "@/features/monthly-billing/server/parseMonthlyBillingMutationBody";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const parsed = parseGenerateZohoMonthlyInvoiceBody(body);
  if (!parsed.ok) {
    return NextResponse.json(
