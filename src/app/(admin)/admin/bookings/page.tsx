...
import { AdminBookingListRow } from "@/components/dashboard/admin/AdminBookingListRow";
import {
...
import { adminBookingListNextAction } from "@/features/dashboards/adminBookingListDisplay";
...
export default async function AdminBookingsPage({ searchParams }: PageProps) {
  ...
  return (
    <AdminDashboardShell
      title="Bookings"
      subtitle="Search and filter — open a booking to act."
      nav={[...ADMIN_DASHBOARD_NAV]}
    >
      {queueCounts.ok ? (
        <AdminBookingsQueuesSummary queues={queueCounts.queues} activeFilter={filter} />
      ) : null}
          ...
          {result.bookings.map((b) => (
            <li key={b.id}>
              <AdminBookingListRow
                ...
                nextAction={adminBookingListNextAction(b)}
              />
            </li>
          ))}
        </ul>
