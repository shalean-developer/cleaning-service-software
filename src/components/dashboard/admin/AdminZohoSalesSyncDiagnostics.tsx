import type { ZohoSalesSyncDiagnosticsResult } from "@/features/zoho-sales-sync/server/loadZohoSalesSyncDiagnostics";

type Props = {
  diagnostics: ZohoSalesSyncDiagnosticsResult;
};
...
export function AdminZohoSalesSyncDiagnostics({ diagnostics }: Props) {
  return (
    <section className="space-y-4 rounded-xl border border-zinc-200 bg-white p-4 shadow-sm">
      <div>
        <h2 className="text-base font-semibold text-zinc-900">Zoho sales sync</h2>
        <p className="mt-1 text-sm text-zinc-600">
          Shalean paid sales recorded in Zoho Books for accounting. Read-only diagnostics.
        </p>
      </div>

      {!diagnostics.enabled ? (
        <p className="rounded-lg bg-zinc-50 px-3 py-2 text-sm text-zinc-700">
          Zoho sales sync is disabled (ZOHO_SALES_SYNC_ENABLED=false).
        </p>
      ) : null}
      ...
      {diagnostics.rows.length === 0 ? (
        <p className="text-sm text-zinc-600">No sales sync rows yet.</p>
      ) : (
          ...
          <table className="min-w-full text-left text-sm">
            <thead className="border-b border-zinc-200 text-xs uppercase tracking-wide text-zinc-500">
              <tr>
                ...
                <th className="px-2 py-2">Zoho invoice</th>
              ...
              {diagnostics.rows.map((row) => (
                <tr key={row.id} className="border-b border-zinc-100 align-top">
                  ...
                  <td className="px-2 py-2">
                    {row.invoiceNumber ?? row.bookingReference?.slice(0, 8) ?? "—"}
                  </td>
                  ...
                  <td className="px-2 py-2 font-mono text-xs">{row.zohoInvoiceId ?? "—"}</td>
