...
function isTechnicalDetail(detail: string): boolean {
  return /^Ref\s/i.test(detail.trim());
}
...
export function CustomerLifecycleTimeline({ events }: Props) {
  if (events.length === 0) {
    return (
  ...
  return (
    <ol className="relative space-y-0">
      {events.map((event, index) => {
        ...
        const repeatsTitle = previous?.title === event.title;
        ...
        return (
          <li key={event.id} className="relative flex gap-3 pb-4 last:pb-0">
            ...
            <section className="min-w-0 flex-1 pb-0.5">
              {repeatsTitle ? (
                <p className="text-xs text-zinc-400">{formatEventTime(event.at)}</p>
              ) : (
                <>
                  <p
              ...
              {repeatsTitle && friendlyDetail ? (
                <p className="mt-0.5 text-xs leading-relaxed text-zinc-500">{friendlyDetail}</p>
              ) : null}
              {technicalDetail ? (
                <p className="mt-1 font-mono text-[11px] text-zinc-400">{technicalDetail}</p>
              ) : null}
            </section>
          </li>
        );
