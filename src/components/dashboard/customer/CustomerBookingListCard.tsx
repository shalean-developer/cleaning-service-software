...
import { UI_INTERACTIVE_LIST_CARD_CLASS } from "@/lib/ui/productUiTokens";
import {
...
export function CustomerBookingListCard({ booking }: Props) {
  ...
  return (
    ...
    >
      <section className="flex flex-col gap-2.5 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
        <section className="min-w-0 flex-1 space-y-2">
          ...
          <p className="text-sm text-zinc-600">
            <span className="font-medium text-zinc-800">{booking.scheduleLabel}</span>
            <span className="text-zinc-400"> · </span>
            <span className="break-words text-zinc-500">{booking.display.locationSummary}</span>
          </p>
